...
def index(request):
    hora = datetime.datetime.now()
    cierre = ('CIERRE'+session['materia']+'-'+str(hora.strftime("%d%m")))
    print("NOMBRE DEL CIERRE",cierre)
    ...
    if usuario is not None:
        ...
        if usuario.is_authenticated:
            ...
            if materiaquery:
                ...
                cierre = ('CIERRE'+session['materia']+'-'+str(hora.strftime("%d%m")))
                print("NOMBRE DEL CIERRE",cierre)
                cierrechk = Asistencia.objects.filter(
                            num_control=cierre, fecha__date=hora, clave_matA=session['materia'])
                print("cierrechk:",cierrechk)
                ...
                if cierrechk:
                    ...
                    nomateria = False
                    return render(request, 'index2.html', {"materia": materia, "horaI": horaI, "inicio": inicio})
                else:
                    print("NO HAY CIERRE")
                    inicio = True
                ...
                if hora <= session["horamax"].replace(tzinfo=None) and inicio == True and hora.time() <= session["horafinal"]: 
                    nomateria = False
                    print("LA MATERIA FINALIZA A LAS FINALIZA A LAS: ", session["horafinal"])
                    print("SE TOMARA LISTA HASTA LAS: ", session["horamax"])
                    return render(request, 'index2.html', {"materia": materia, "horaI": horaI, "horaF": horaF,"inicio":inicio,"horamax": session["horamax"]})
                else:
...
def loginalumno(request):
    ...
    cierrechk = Asistencia.objects.filter(
                num_control=nombreinicio, fecha__date=hora, clave_matA=session['materia']).first
    if cierrechk == None:
        print("NO HAY INICIO")

    if materiaquery and hora <= session["horamax"].replace(tzinfo=None) and hora.time() <= session["horafinal"] and cierrechk:
        inicio = True
        print("MATERIA A ESTA HORA: ", materiaquery.clave_mat)
...
def resumen(request):
    ...
    asistenciaC = Asistencia.objects.filter(
        clave_matA=materia_clave, fecha__day=dia_select).order_by('fecha').exclude(num_control__contains="CIERRE")  # ALUMNOS DEL DIA
...
def resumenPOST(request):
    ...
    if request.method == "POST":
        ...
        asistenciaC = Asistencia.objects.filter(
                clave_matA=session['materia'], fecha__day=dia_select).order_by('fecha').exclude(num_control__contains="CIERRE") # ALUMNOS DEL DIA
